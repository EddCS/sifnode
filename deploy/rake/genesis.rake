...
def network_create(chainnet:, validator_count:, build_dir:, seed_ip_address:, network_config:)
  puts "sifgen network create #{chainnet} #{validator_count} #{build_dir} #{seed_ip_address} #{network_config}"
  result = safe_system("sifgen network create #{chainnet} #{validator_count} #{build_dir} #{seed_ip_address} #{network_config}")
end
...
def boot_docker_network(chainnet:, seed_network_address:, eth_config:)
  ...
  if chainnet == 'localnet'
    ...
    cmd += "IPV4_SUBNET=#{seed_network_address} #{eth_config} docker-compose -f #{cwd}/../genesis/docker-compose-integration.yml up -d | tee #{cwd}/../../log/#{chainnet}.log"
  else
